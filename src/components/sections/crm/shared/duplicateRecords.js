...
async function findMatchesForType(supabase, check) {
  if (check.type === 'company')
    return findCompanyMatches(supabase, check.record);
  if (check.type === 'contact')
    return findContactMatches(supabase, check.record);
    ...
    .map((candidate) => {
      ...
      return reasons.length
        ? {
            ...
            subtitle: [candidate.city, candidate.region, candidate.postal_code]
              .filter(Boolean)
              .join(', '),
            reasons,
            record: candidate,
...
async function findContactMatches(supabase, contact) {
  ...
  if (
    !hasText(fullName) &&
    !hasText(contact.accountNumber) &&
    !hasText(contact.email) &&
    !hasText(contact.phone) &&
    !hasText(contact.mobilePhone)
  ) {
    return [];
  }
  ...
  const { data, error } = await supabase
    .from('contacts')
    .select(
      'id, first_name, last_name, title, account_number, email, phone, mobile_phone, companies(id, name)',
    )
    .limit(300);
  ...
  return (data || [])
    .map((candidate) => {
      ...
      const reasons = [
        ...
        sameValue(contact.accountNumber, candidate.account_number) &&
          'same account number',
        sameValue(contact.email, candidate.email) && 'same email',
        samePhone(contact.phone, candidate.phone) && 'same phone',
        samePhone(contact.mobilePhone, candidate.mobile_phone) &&
          'same mobile phone',
      ].filter(Boolean);
    ...
    .map((candidate) => {
      ...
      const reasons = [
        lead.contactId &&
          ...
          'same contact has an open lead',
        lead.companyId &&
          lead.companyId === candidate.company_id &&
        ...
        hasText(lead.source) &&
          ...
          'same source',
        hasText(lead.source) &&
          similarText(lead.source, candidate.source) &&
          'similar source',
      ].filter(Boolean);
...
function similarText(left, right) {
  ...
  if (leftText.includes(rightText) || rightText.includes(leftText))
    return Math.min(leftText.length, rightText.length) >= 5;
