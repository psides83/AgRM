          ...
          >
            <Box sx={{ minWidth: 0 }}>
              ...
              <Typography variant="body1" sx={{ color: 'text.secondary' }}>
                {entityName(deal)}
                {deal.companies?.name ? ` · ${deal.companies.name}` : ''}
              </Typography>
            </Box>
            <Stack
              direction="column"
      ...
      <Grid size={{ xs: 12, lg: 8 }}>
        <Stack direction="column" spacing={3}>
          ...
          <EquipmentCard
            ...
            onAdd={() => setDialog('equipment')}
          />
          <CrmFilesPanel recordType="deal" recordId={deal.id} />
          <TimelineCard
            items={timelineItems}
            onAddActivity={() => setDialog('activity')}
  ...
  return (
    <Paper sx={{ p: { xs: 3, md: 4 } }}>
      ...
      <Stack direction="column" spacing={1.5}>
        <RecordRow
          title={
            deal.contacts ? contactName(deal.contacts) : 'No linked contact'
          }
          subtitle={
            deal.contacts
              ? [
                  deal.contacts.account_number,
                  deal.contacts.title,
                  deal.contacts.email,
                  formatPhone(
                    deal.contacts.mobile_phone || deal.contacts.phone,
                  ),
                ]
                  .filter(Boolean)
                  .join(' · ')
              : 'Add a contact link later from edit support.'
          }
        ...
        <RecordRow
          ...
          subtitle={
            deal.companies
              ? [
                  formatEnum(deal.companies.company_type),
                  deal.companies.city,
                  deal.companies.region,
                ]
                  .filter(Boolean)
                  .join(' · ')
              : 'Company linkage is optional.'
          }
        ...
        <RecordRow
          ...
          subtitle={
            deal.leads
              ? [
                  deal.leads.source,
                  `Priority ${deal.leads.priority || '-'}`,
                  `Budget ${formatCurrency(deal.leads.estimated_budget)}`,
                ]
                  .filter(Boolean)
                  .join(' · ')
          ...
          return (
            ...
            >
              <Chip
                ...
                size="small"
                color={
                  isComplete ? 'success' : isCurrent ? 'primary' : 'neutral'
                }
                variant="soft"
                sx={{ minWidth: 74 }}
              />
