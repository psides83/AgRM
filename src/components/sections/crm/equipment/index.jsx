...
const Equipment = () => {
  ...
  const fetchOptions = async () => {
    const [contactsResult, leadsResult, dealsResult] = await Promise.all([
      supabase.from('contacts').select('id, first_name, last_name, company_id, companies(id, name)').order('last_name', { ascending: true }).limit(300),
      supabase.from('leads').select('id, source, status, contact_id, company_id, contacts(id, first_name, last_name), companies(id, name)').order('created_at', { ascending: false }).limit(300),
      supabase.from('deals').select('id, name, stage, contact_id, company_id, lead_id, contacts(id, first_name, last_name), companies(id, name)').order('updated_at', { ascending: false }).limit(300),
    ]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'equipment_interests' }, () => fetchEquipment())
      .subscribe();
  ...
  const handleQuickUpdate = async (itemId, field, value) => {
    const { error: updateError } = await supabase.from('equipment_interests').update({ [field]: value }).eq('id', itemId);
    if (updateError) {
  ...
  const handleDelete = async () => {
    ...
    const { error: deleteError } = await supabase.from('equipment_interests').delete().eq('id', deleteItem.id);
    ...
    <Grid container spacing={3}>
      <Grid size={12}>
        <PageHeader
          ...
          actionComponent={
            <Button variant="contained" onClick={openCreateDialog} startIcon={<IconifyIcon icon="material-symbols:add-rounded" />}>
      ...
      <Grid size={12}>
        <Paper sx={{ p: { xs: 2, md: 3 } }}>
          <Stack direction={{ xs: 'column', xl: 'row' }} spacing={2}>
            <TextField label="Search Equipment" value={filters.search} onChange={handleFilter('search')} placeholder="Make, model, stock, serial, customer..." fullWidth />
            <FilterSelect label="Category" value={filters.category} onChange={handleFilter('category')} options={equipmentCategories} />
            <FilterSelect label="Condition" value={filters.condition} onChange={handleFilter('condition')} options={equipmentConditions} />
            <FilterSelect label="Availability" value={filters.availability} onChange={handleFilter('availability')} options={equipmentAvailability} />
            <FilterSelect label="Status" value={filters.status} onChange={handleFilter('status')} options={equipmentStatuses} />
            <TextField select label="Trade-in" value={filters.tradeIn} onChange={handleFilter('tradeIn')} sx={{ minWidth: 140 }}>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  filteredEquipment.map((item) => (
                    <EquipmentRow key={item.id} item={item} onQuickUpdate={handleQuickUpdate} onEdit={openEditDialog} onDelete={setDeleteItem} />
                  ))
                ) : (
      ...
      <Dialog open={Boolean(deleteItem)} onClose={() => setDeleteItem(null)} fullWidth maxWidth="xs">
        <DialogTitle>Delete Equipment Interest?</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            This removes {deleteItem ? equipmentName(deleteItem) : 'this equipment interest'} from the CRM record.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button color="neutral" onClick={() => setDeleteItem(null)}>Cancel</Button>
          <Button color="error" variant="contained" loading={isDeleting} onClick={handleDelete}>Delete</Button>
        </DialogActions>
      </Dialog>
    </Grid>
  );
...
function EquipmentRow({ item, onQuickUpdate, onEdit, onDelete }) {
  ...
  return (
    <TableRow hover>
        ...
        <Stack direction="row" spacing={1.5} alignItems="center">
          ...
          <Box sx={{ minWidth: 0 }}>
            ...
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>
              {[formatEnum(item.category), formatEnum(item.condition), item.stock_number ? `Stock ${item.stock_number}` : null, item.serial_number ? `Serial ${item.serial_number}` : null].filter(Boolean).join(' · ')}
            </Typography>
          </Box>
        </Stack>
      </TableCell>
      ...
      <TableCell sx={{ minWidth: 190 }}>
        <TextField select size="small" value={item.availability || 'availability_unknown'} onChange={(event) => onQuickUpdate(item.id, 'availability', event.target.value)} fullWidth>
          {equipmentAvailability.map((availability) => <MenuItem key={availability} value={availability}>{formatEnum(availability)}</MenuItem>)}
        </TextField>
      </TableCell>
      <TableCell sx={{ minWidth: 190 }}>
        <TextField select size="small" value={item.status || 'not_started'} onChange={(event) => onQuickUpdate(item.id, 'status', event.target.value)} fullWidth>
      ...
      <TableCell>
        <Stack direction="row" spacing={1} alignItems="center">
          <Chip label={item.trade_in ? 'Yes' : 'No'} size="small" variant="soft" color={item.trade_in ? 'warning' : 'neutral'} />
          <Button size="small" color="neutral" onClick={() => onEdit(item)} startIcon={<IconifyIcon icon="material-symbols:edit-outline-rounded" />}>
            Edit
          </Button>
          <Button size="small" color="error" onClick={() => onDelete(item)} startIcon={<IconifyIcon icon="material-symbols:delete-outline-rounded" />}>
            Delete
          </Button>
        </Stack>
      </TableCell>
...
function FilterSelect({ label, value, onChange, options }) {
  return (
    <TextField select label={label} value={value} onChange={onChange} sx={{ minWidth: 170 }}>
      <MenuItem value="all">All</MenuItem>
      {options.map((option) => <MenuItem key={option} value={option}>{label === 'Category' ? <CategoryOption category={option} /> : formatEnum(option)}</MenuItem>)}
    </TextField>
  );
}
...
function AddEquipmentDialog({ open, item, contacts, leads, deals, onClose, onSaved, supabase }) {
  const [form, setForm] = useState(emptyForm);
  const [isSaving, setIsSaving] = useState(false);
  const [error, setError] = useState(null);
  const isEditing = Boolean(item);

  const options = form.relatedType === 'lead' ? leads : form.relatedType === 'deal' ? deals : contacts;
  const selectedRecord = options.find((option) => option.id === form.relatedId) || null;
  ...
  const handleSave = async () => {
    ...
    const payload = {
      contact_id: form.relatedType === 'contact' ? form.relatedId : selectedRecord?.contact_id || null,
      lead_id: form.relatedType === 'lead' ? form.relatedId : selectedRecord?.lead_id || null,
    ...
    const { error: saveError } = isEditing
      ? await supabase.from('equipment_interests').update(payload).eq('id', item.id)
      : await supabase.from('equipment_interests').insert({ ...payload, owner_id: user.id });
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      <DialogTitle>{isEditing ? 'Edit Equipment Interest' : 'Add Equipment Interest'}</DialogTitle>
      <DialogContent>
        <Stack direction="column" spacing={2} sx={{ pt: 1 }}>
          ...
          {!isEditing && (
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              ...
              <Autocomplete
                ...
                onChange={(_event, value) => setForm((prev) => ({ ...prev, relatedId: value?.id || '' }))}
                getOptionLabel={(option) => relatedOptionLabel(option, form.relatedType)}
                isOptionEqualToValue={(option, value) => option.id === value.id}
                renderInput={(params) => <TextField {...params} label="Related Record" placeholder="Search" />}
                fullWidth
              />
            </Stack>
          )}
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            <TextField select label="Category" value={form.category} onChange={handleField(setForm, 'category')} fullWidth>
              {equipmentCategories.map((category) => <MenuItem key={category} value={category}><CategoryOption category={category} /></MenuItem>)}
            </TextField>
      ...
      <DialogActions>
        ...
        <Button variant="contained" onClick={handleSave} loading={isSaving}>{isEditing ? 'Save Changes' : 'Save Interest'}</Button>
      </DialogActions>
    </Dialog>
  );
}
...
function formFromEquipment(item) {
  return {
    ...
    availability: item.availability || 'availability_unknown',
...
function equipmentOwner(item) {
  ...
  if (item.leads) return item.leads.source || contactName(item.leads.contacts) || item.leads.companies?.name || 'Lead';
...
function relatedOptionLabel(option, type) {
  if (type === 'deal') return [option.name, option.companies?.name].filter(Boolean).join(' - ');
  if (type === 'lead') return [option.source || 'Lead', contactName(option.contacts), option.companies?.name].filter(Boolean).join(' - ');
  return [contactName(option), option.companies?.name].filter(Boolean).join(' - ');
}
...
function equipmentBudget(item) {
  ...
  return [formatCurrency(item.price_min), formatCurrency(item.price_max)].filter((value) => value !== '-').join(' - ') || '-';
}
