...
async function getSearchResults(rawQuery) {
  ...
  const [
    ...
    supabase
      .from('contacts')
      .select(
        'id, first_name, last_name, title, account_number, email, phone, mobile_phone, city, region, companies(id, name)',
      )
      .or(
        `first_name.ilike.${pattern},last_name.ilike.${pattern},account_number.ilike.${pattern},email.ilike.${pattern},phone.ilike.${pattern},mobile_phone.ilike.${pattern},title.ilike.${pattern}`,
      )
    ...
    supabase
      ...
      .or(
        `name.ilike.${pattern},company_type.ilike.${pattern},email.ilike.${pattern},phone.ilike.${pattern},city.ilike.${pattern},region.ilike.${pattern}`,
      )
    ...
    supabase
      .from('leads')
      .select(
        'id, source, account_number, status, priority, estimated_budget, next_follow_up_at, notes, contacts(id, first_name, last_name), companies(id, name)',
      )
      .or(
        `source.ilike.${pattern},account_number.ilike.${pattern},notes.ilike.${pattern}`,
      )
    ...
    supabase
      ...
      .or(
        `make.ilike.${pattern},model.ilike.${pattern},stock_number.ilike.${pattern},serial_number.ilike.${pattern},notes.ilike.${pattern}`,
      )
      .order('created_at', { ascending: false })
      .limit(25),
  ]);
...
const CRMSearch = async ({ query: rawQuery }) => {
  ...
  try {
    ...
    return (
      <Alert severity="error">Could not run CRM search. {error.message}</Alert>
    );
  }
    ...
    <Grid container spacing={3}>
      ...
      <Grid size={12}>
        <Typography variant="body2" sx={{ color: 'text.secondary' }}>
          {query
            ...
            : 'Enter a search term to find CRM records.'}
        </Typography>
      </Grid>
            ...
            results.companies.map((company) => (
              <ResultRow
                ...
                subtitle={[
                  ...
                  company.city,
                  company.region,
                  company.email || formatPhone(company.phone),
                ]
                  .filter(Boolean)
                  .join(' · ')}
                chip="Company"
            ...
            results.leads.map((lead) => (
              <ResultRow
                ...
                subtitle={[
                  lead.source,
                  lead.account_number,
                  `Budget ${formatCurrency(lead.estimated_budget)}`,
      ...
      <Grid size={12}>
        <ResultGroup
          ...
          count={results.equipment.length}
        >
          {results.equipment.length ? (
            results.equipment.map((interest) => (
              <ResultRow
                key={interest.id}
...
function equipmentName(interest) {
  return (
    [interest.model_year, interest.make, interest.model]
      .filter(Boolean)
      .join(' ') ||
    interest.stock_number ||
    interest.serial_number ||
    formatEnum(interest.category)
  );
}
...
function equipmentOwner(interest) {
  ...
  if (interest.leads)
    return interest.leads.source || interest.leads.companies?.name || 'Lead';
