...
function EditCompanyDialog({
  ...
  const [error, setError] = useState(null);
  const selectedCopyContact =
    contacts.find((contact) => contact.id === form.copyContactId) ||
    contacts[0] ||
    null;
  ...
  const handleCopyContactChange = (event) => {
    const copyContactId = event.target.value;
    const nextContact = contacts.find(
      (contact) => contact.id === copyContactId,
    );
    ...
    setForm((prev) => ({
      ...
      ...(prev.sameAddressAsContact
        ...
        : {}),
      ...(prev.sameCoordinatesAsContact
        ? fieldsFromContact(nextContact, coordinateFields)
        : {}),
    }));
  };

  const handleUseContactField = (key, fields) => (event) => {
    const checked = event.target.checked;
    setForm((prev) => ({
      ...prev,
      [key]: checked,
      ...(checked ? fieldsFromContact(selectedCopyContact, fields) : {}),
    }));
  };
        ...
        <Stack direction="column" spacing={2} sx={{ pt: 1 }}>
          ...
          {contacts.length > 0 && (
            <TextField
              ...
              value={form.copyContactId}
              onChange={handleCopyContactChange}
              fullWidth
            >
              {contacts.map((contact) => (
...
function DeleteCompanyDialog({ open, company, supabase, onClose, onDeleted }) {
...
function AddContactDialog({ open, company, onClose, onSaved, supabase }) {
  ...
  const handleUseCompanyField = (key, fields) => (event) => {
    ...
    setForm((prev) => ({
      ...prev,
      [key]: checked,
      ...(checked ? fieldsFromCompany(company, fields) : {}),
    }));
  };
